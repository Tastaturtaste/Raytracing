...
std::optional<IntersectionTrace> Sphere::intersect_impl(const Ray& r) const noexcept {
	...
	if (inside)
		normal = -normal;
	return { { .hit_position = hitPosition, .normal = normal, .distance = t, .material = material(), .inside = inside } };
}