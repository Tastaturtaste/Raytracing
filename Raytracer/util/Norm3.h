...
public:
	...
	explicit norm3(const glm::dvec3& v) noexcept
	...
	inline static std::array<norm3,3> orthogonalFromZ(const norm3& z) noexcept {
	...
	inline static std::array<norm3, 3> orthogonalFromZY(const norm3& z, const norm3& y) noexcept {
		const auto x = glm::normalize(glm::cross(y.vec_, z.vec_));
