...
public:
	Renderer(Scene const& scene, const RenderParams& renderParams)
		: scene_(&scene), renderParams_(renderParams), normalizer_(1. / renderParams.sizeX),
...
template<class Scene>
Color Renderer<Scene>::radiance(const Ray& r, std::mt19937& rnd, unsigned int depth) {
...
template<class Scene>
auto Renderer<Scene>::generateRenderJob(std::size_t sample) {
...
template<class Scene>
std::vector<Color> Renderer<Scene>::render2() {
...
template<class Scene>
std::vector<Color> Renderer<Scene>::render() {
